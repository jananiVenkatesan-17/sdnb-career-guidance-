...
async function showMockInterviewQuestions() {
  ...
  try {
    const courseId = getSelectedCourseId();
    const res = await fetch(`${API_MOCK_INTERVIEW}?courseId=${encodeURIComponent(courseId)}`);
    const json = await res.json();

    if (!json || json.status !== "success") {
      chat.innerHTML = `<div class="bot-msg">❌ ${json.message || "Mock interview questions not loading."}</div>`;
      return;
    }
...
async function showCodingTopics() {
  ...
  html += `
    <div class="bot-msg">
      ...
      ${item.description}<br>
      <a href="${item.link}" target="_blank">Practice Link</a>
    </div>
  `;
});
